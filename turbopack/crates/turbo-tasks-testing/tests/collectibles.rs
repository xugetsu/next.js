...
use turbo_tasks::{emit, CollectiblesSource, RcStr, ValueToString, Vc, VcOperation};
...
async fn transitive_emitting() {
    run(&REGISTRATION, || async {
        ...
        let list = VcOperation::new(result).peek_collectibles::<Box<dyn ValueToString>>();
...
async fn taking_collectibles() {
    run(&REGISTRATION, || async {
        ...
        let list = VcOperation::new(result).take_collectibles::<Box<dyn ValueToString>>();
...
async fn my_collecting_function() -> Result<Vc<Thing>> {
    let result = my_transitive_emitting_function("".into(), "".into());
    VcOperation::new(result).take_collectibles::<Box<dyn ValueToString>>();
    Ok(result)
}
...
async fn my_transitive_emitting_function_collectibles(key: RcStr, key2: RcStr) -> Vc<Collectibles> {
    ...
    Vc::cell(VcOperation::new(result).peek_collectibles::<Box<dyn ValueToString>>())
}
...
async fn my_transitive_emitting_function_with_child_scope(
    ...
    let list = VcOperation::new(thing).peek_collectibles::<Box<dyn ValueToString>>();
    assert_eq!(list.len(), 2);
    Ok(thing)
}
